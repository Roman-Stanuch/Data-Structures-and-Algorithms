...
// Destroys a hashmap, freeing its memory. The map should not be used after
// calling this function.
void HashmapDestroy(hashmap* map);

// Adds the given "key" and "value" pair to "map" if there is room and the key
// does not already exist. Returns 1 if success and 0 if failure.
int HashmapAdd(hashmap* map, const char* key, const char* value);
// Removes the "key" and its associated value from "map". Returns 1 if success
// and 0 if no matching key was found.
int HashmapRemove(hashmap* map, const char* key);

// Returns the associated value for the given "key" in "map". If none exists,
// returns "".
